...
- (NSString *)stringFromNumber:(NSNumber *)number numberFormatter:(NSNumberFormatter *)formattor{
    ...
    NSString *formatStr = [formattor stringFromNumber:number];
    return formatStr;
}
...
- (NSString *)finalString:(NSNumber *)number stringFormat:(NSString *)formatStr andFormatter:(NSNumberFormatter *)formatter{
    ...
    if(formatter){
        if ([formatStr rangeOfString:@".2f"].length > 0) {
            NSString *str = HTSTR(@"%.3f", [number floatValue]);
            str = [str substringToIndex:str.length - 1];
            finalString = [str formatNumberString];
        }else {
